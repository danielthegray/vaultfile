...
fn check_file_overwrite(file_to_check: &Path, overwrite_no: bool) {
    if file_to_check.exists() {
        if overwrite_no {
            eprintln!("File at '{}' already exists, and no overwriting has been selected. Doing nothing...", file_to_check.to_str().unwrap());
            exit(exitcode::OK);
        }
        match read_user_confirmation(format!(
            "File at '{}' already exists. Overwrite?",
            file_to_check.to_str().unwrap()
        )) {
...
fn load_or_create_vaultfile(vaultfile_path: &Path) -> Vaultfile {
    load_vaultfile(vaultfile_path, false)
}

fn load_existing_vaultfile(vaultfile_path: &Path) -> Vaultfile {
    load_vaultfile(vaultfile_path, true)
}

fn load_vaultfile(vaultfile_path: &Path, must_exist: bool) -> Vaultfile {
    match Vaultfile::load_from_file(vaultfile_path) {
        ...
        Err(error) => match error.kind {
            VaultfileErrorKind::VaultfileNotFound => {
                if must_exist {
                    eprintln!(
                        "No vaultfile found at {}!",
                        vaultfile_path.to_str().unwrap()
                    );
...
fn build_key_path(key_path: Option<&str>) -> PathBuf {
    if key_path.is_some() {
        let key_path_string = String::from(key_path.unwrap());
        let key_path = Path::new(&key_path_string);
        if key_path.is_dir() {
            let default_key_name = format!("{}.key", &get_username());
            return key_path.join(default_key_name);
        }
        if key_path.is_file() {
            return key_path.to_path_buf();
        }
    }
    Path::new(&get_default_vaultfile_folder()).join(format!("{}.key", &get_username()))
}

fn generate_key_command(cli_call: &ArgMatches) {
    let key_path = build_key_path(cli_call.value_of("key-path"));
    // it NEEDS to be pointing to a key FILE
    assert_eq!(key_path.is_file(), true);
    ensure_folder_exists(key_path.parent().unwrap());
...
fn register_key_command(cli_call: &ArgMatches) {
    ...
    let vaultfile_path = Path::new(cli_call.value_of("file").unwrap());
    ...
    let new_key = match cli_call.value_of("key-json") {
        ...
        None => match cli_call.value_of("key-file") {
            Some(json_filename) => load_public_key(Path::new(json_filename)),
            None => {
    ...
    let private_key_path = Path::new(&get_default_vaultfile_folder()).join(format!(
        "{}.key",
    ...
    let new_public_key = match new_key {
    ...
    if !cli_call.is_present("overwrite-yes") {
        if vaultfile.is_key_registered(key_name) {
            let vaultfile_path_string = vaultfile_path.to_str().unwrap();
            if cli_call.is_present("overwrite-no") {
                eprintln!("The vaultfile at '{}' already has a key registered under name {}, and no overwriting has been selected. Doing nothing...", vaultfile_path_string, &key_name);
                exit(exitcode::OK);
            }
            match read_user_confirmation(format!(
                "Vaultfile at '{}' already has a key registered under name {}. Overwrite?",
                vaultfile_path_string, &key_name
            )) {
                Confirmation::YES => return,
                Confirmation::NO => {
                    eprintln!("The vaultfile at '{}' already has a key registered under name {}, and no overwriting has been selected. Doing nothing...", vaultfile_path_string, &key_name);
    ...
    vaultfile.register_key(&key_name, new_public_key, &private_key).unwrap_or_else(|error| {
        match error.kind {
    ...
    vaultfile
        .save_to_file(Path::new(vaultfile_path))
...
fn add_secret_command(cli_call: &ArgMatches) {
    ...
    if !cli_call.is_present("overwrite-yes") {
        if vaultfile.has_secret_named(secret_name) {
            let vaultfile_path_string = vaultfile_path.to_str().unwrap();
            if cli_call.is_present("overwrite-no") {
                eprintln!("The vaultfile at '{}' already has a secret registered under name {}, and no overwriting has been selected. Doing nothing...", vaultfile_path_string, &secret_name);
                exit(exitcode::OK);
            }
            match read_user_confirmation(format!(
                "Vaultfile at '{}' already has a secret registered under name {}. Overwrite?",
                vaultfile_path_string, &secret_name
            )) {
                Confirmation::YES => return,
                Confirmation::NO => {
                    eprintln!("The vaultfile at '{}' already has a secret registered under name {}, and no overwriting has been selected. Doing nothing...", vaultfile_path_string, &secret_name);
...
fn read_secret_command(cli_call: &ArgMatches) {
    ...
    let private_key_path = match cli_call.value_of("key-file") {
        Some(key_path) => Path::new(key_path).to_path_buf(),
        None => {
            let key_name = cli_call.value_of("key-name").unwrap();
            Path::new(&get_default_vaultfile_folder()).join(format!("{}.key", &key_name))
        }
    };
    let private_key = match load_private_key(&private_key_path) {
        Ok(key) => key,
    ...
    let cli_call = App::new("vaultfile")
        ...
        .subcommand(
            SubCommand::with_name("register-key")
                ...
                .arg(
                    ...
                    .long("file")
                    .takes_value(true)
                    .default_value(default_vaultfile_name)
        ...
        .subcommand(
            ...
            .arg(
                ...
                .help("The secret value to store in the vaultfile (assumed to be a UTF-8-encoded string).")
            )
            .arg(
